...
export function AdminPage() {
  ...
  const fetchEvents = async () => {
    try {
      const res = await axios.get("http://localhost:8081/api/v1/registration");
  ...
  const handleSubmit = async (values: EventFormValues) => {
    try {
      if (editing) {
        const res = await axios.put(`http://localhost:8081/api/v1/registration/${editing.id}`, values);
        if (res.data && res.data.success) {
        ...
        const res = await axios.post(`http://localhost:8081/api/v1/registration`, values);
        if (res.data && res.data.success) {
                  ...
                  <TableCell>
                    <Switch
                      ...
                      onCheckedChange={async () => {
                        try {
                          await axios.patch(`http://localhost:8081/api/v1/registration/${e.id}/toggle`);
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={async () => {
                if (confirmDelete) {
                  try {
                    await axios.delete(`http://localhost:8081/api/v1/registration/${confirmDelete.id}`);
